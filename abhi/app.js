...
(function() {
    ...
    function loadCSS(url) {
      var link = shadow.createElement('link');
      link.rel = 'stylesheet';
      link.href = url;
      shadow.head.appendChild(link);
    }
  ...
  wrapper.innerHTML = `<link rel="stylesheet" href="https://dasmeet9.github.io/model/abhi/style.css">
    ...
    //   var container = shadow.getElementById(config.containerId);
      let container = document.getElementById(config.containerId);
      if (!container) {
        container = shadow.createElement('div');
        container.id = config.containerId;
        shadow.body.appendChild(container);
      }
    ...
    //   createWidgetHTML(container, config);
      ...
      const registrationForm = shadow.getElementById("registration-form");
      const chatInterface = shadow.getElementById("chat-interface");
      ...
      const fetchAvatarData = async () => {
          ...
          if (!result.success) {
              ...
              shadow.getElementById("chat-button").style.display = "none";
              return;
          }
      ...
      (async () => {
          ...
          shadow.getElementById("avatar-logo").src = avatarData.data.logo;
          if (avatarData.data.logo && avatarData.data.logo.length > 0) {
            shadow.getElementById("avatar-logo").src = avatarData.data.logo;
          } else {
              // shadow.getElementById("avatar-logo").src = "https://dasmeet9.github.io/model/NiftyHMS%20(1).png"; // Default Image
              shadow.getElementById("avatar-logo").src = "https://dasmeet9.github.io/model/unnamed.png"; // Default Image
      ...
      function stripHtmlTags(html) {
          const temp = shadow.createElement('div');
          temp.innerHTML = html;
      ...
      function hideCommandListeningIndicator() {
          const indicator = shadow.getElementById('command-listening-indicator');
          if (indicator) {
      ...
      async function processNextQuestion(text) {
          ...
          const chatBubbles = shadow.querySelectorAll('.chat-bubble.user');
          const lastUserBubble = chatBubbles.length > 0 ? chatBubbles[chatBubbles.length - 1] : null;
      ...
      function appendChatBubble(sender, message) {
        const bubble = shadow.createElement("div");
      ...
      function showLoadingIndicator() {
          const loadingBubble = shadow.createElement('div');
   ...
   const chatbotContainer = shadow.createElement("div");
   chatbotContainer.id = "chatbot-container";
   shadow.body.appendChild(chatbotContainer);
